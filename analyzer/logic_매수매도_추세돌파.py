...
def judge_매수신호(df_기준봉):
    ...
    b_강력돌파 = (n_종가 > n_고가20) and (n_거래량 > 3.5 * n_거래량ma20)
    # b_변동성수축 = (n_고가5 - n_저가5) / n_종가 < 0.01
    # b_변동성수축 = (n_고가5 - n_저가5) / n_종가 * 100 < 1.2
    b_변동성수축 = (n_고가5 - n_저가5) / n_종가 * 100 < 0.9
    ...
    b_돌파검증 = b_강력돌파 and b_변동성수축 and (n_이격도20 < 101.5)
    # b_돌파검증 = b_강력돌파 and b_변동성수축 and (n_이격도20 < 102)
...
def judge_매도신호(df_기준봉, dic_args):
    ...
    b_고가이탈 = (n_종가 < n_고가한계) and (n_분봉최고수익률 > 1.5)
    # b_고가이탈 = False
    ...
    n_타임아웃 = 300 if '초봉' in s_봉수 else 900 if '분봉' in s_봉수 else 0
    ...
    b_타임아웃 = (n_경과시간 > n_타임아웃 and n_거래량 > 0) or (n_경과시간 > 180 and n_수익률 < 0.2) or s_탐색시간 > '15:15:00'
