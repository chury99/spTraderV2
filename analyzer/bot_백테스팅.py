...
class AnalyzerBot:
    ...
    def make_매매신호(self, s_봉수):
        ...
        for s_일자 in li_대상일자:
            ...
            # 분봉 데이터 불러오기
            if '분봉' in s_봉수:
                folder_분봉 = os.path.join(self.folder_차트캐시, f'분봉{s_봉수.replace('분봉', '')}')
            ...
            # 매개변수 정의 - 종목별 함수 전달용
            li_매개변수 = [dict(s_종목코드=s_종목코드, s_종목명=dic_코드2종목[s_종목코드], s_봉수=s_봉수, s_일자=s_일자,
                            folder_타겟=folder_타겟, file_타겟=file_타겟, li_매도사유=self.li_매도사유, dic_일봉시가=dic_일봉시가,
                            df_분봉=dic_분봉.get(s_종목코드, pd.DataFrame()),
            ...
            li_df매매신호 = list()
            if self.b_디버그모드:
                for dic_매개변수 in tqdm(li_매개변수, desc=f'매매신호-{s_봉수}-{s_일자}', file=sys.stdout):
                ...
                with mp.Pool(processes=self.n_멀티코어수) as pool:
                    li_df매매신호 = list(tqdm(pool.imap_unordered(self._make_매매신호_종목, li_매개변수),
                                          total=len(li_매개변수), desc=f'매매신호-{s_봉수}-{s_일자}', file=sys.stdout))
            dic_매매신호 = dict(li_df매매신호)

            # 결과파일 저장
            pd.to_pickle(dic_매매신호, os.path.join(folder_타겟, f'{file_타겟}_{s_일자}_{s_봉수}.pkl'))

            # 5초봉 데이터 저장 - 대상종목만
            dic_5초봉 = pd.read_pickle(os.path.join(self.folder_차트캐시, '초봉5', f'dic_차트캐시_5초봉_{s_일자}.pkl'))
            dic_5초봉_대상 = {종목코드 : df_5초봉 for 종목코드, df_5초봉 in dic_5초봉.items() if 종목코드 in li_대상종목}
            folder = f'{folder_타겟}_대상종목5초봉'
            os.makedirs(folder, exist_ok=True)
            pd.to_pickle(dic_5초봉_대상, os.path.join(folder, f'dic_5초봉_대상종목만_{s_일자}.pkl'))

            # 로그 기록
            self.make_로그(f'{s_일자} 완료\n - {len(dic_매매신호):,.0f}개 종목')

    def make_매수매도(self, s_봉수):
        ...
        # 대상일자 확인
        li_전체일자 = sorted(re.findall(r'\d{8}', 파일)[0] for 파일 in os.listdir(folder_소스)
                         if file_소스 in 파일 and s_봉수 in 파일)
        li_완료일자 = [re.findall(r'\d{8}', 파일)[0] for 파일 in os.listdir(folder_타겟)
                   if file_타겟 in 파일 and s_봉수 in 파일 and '.pkl' in 파일]
        li_대상일자 = [일자 for 일자 in li_전체일자 if 일자 not in li_완료일자]

        # 일자별 매수매도 정보 생성
        for s_일자 in li_대상일자:
            # 소스파일 불러오기
            dic_매매신호 = pd.read_pickle(os.path.join(folder_소스, f'{file_소스}_{s_일자}_{s_봉수}.pkl'))
            ...
            # 결과파일 저장
            Tool.df저장(df=df_매수매도, path=os.path.join(folder_타겟, f'{file_타겟}_{s_일자}_{s_봉수}'))
    ...
    def make_매매내역(self, s_봉수):
        ...
        # 대상일자 확인
        li_전체일자 = sorted(re.findall(r'\d{8}', 파일)[0] for 파일 in os.listdir(folder_소스)
                         if file_소스 in 파일 and s_봉수 in 파일 and '.pkl' in 파일)
        li_완료일자 = [re.findall(r'\d{8}', 파일)[0] for 파일 in os.listdir(folder_타겟)
                   if file_타겟 in 파일 and s_봉수 in 파일 and '.pkl' in 파일]
        li_대상일자 = [일자 for 일자 in li_전체일자 if 일자 not in li_완료일자]
        ...
        for s_일자 in li_대상일자:
            # 소스파일 불러오기
            df_매수매도 = pd.read_pickle(os.path.join(folder_소스, f'{file_소스}_{s_일자}_{s_봉수}.pkl'))
            ...
            # 결과파일 저장
            Tool.df저장(df=df_매매내역, path=os.path.join(folder_타겟, f'{file_타겟}_{s_일자}_{s_봉수}'))

            # 리포트 생성 및 저장 - 일별
            df_일별리포트 = self._generate_일별리포트(df_매매내역=df_매매내역)
            folder_일별리포트 = f'{folder_타겟}_일별리포트'
            os.makedirs(folder_일별리포트, exist_ok=True)
            Tool.df저장(df=df_일별리포트, path=os.path.join(folder_일별리포트, f'{file_타겟}_일별리포트_{s_일자}_{s_봉수}'))

            # 리포트 생성 - 누적, 수익금액
            dic_매개변수=dict(s_일자=s_일자, s_봉수=s_봉수, folder_타겟=folder_타겟, file_타겟=file_타겟)
            ...
            Tool.df저장(df=df_수익금액, path=os.path.join(folder_수익금액, f'{file_타겟}_수익금액_{s_일자}_{s_봉수}'))

            # 누적 리포트 저장
            folder_누적리포트 = f'{folder_타겟}_누적리포트'
            os.makedirs(folder_누적리포트, exist_ok=True)
            Tool.df저장(df=df_누적리포트, path=os.path.join(folder_누적리포트, f'{file_타겟}_누적리포트_{s_일자}_{s_봉수}'))
    ...
    def make_수익내역(self, s_봉수):
        ...
        for s_일자 in li_대상일자:
            # 소스파일 불러오기
            df_매매내역 = pd.read_pickle(os.path.join(folder_소스, f'{file_소스}_{s_일자}_{s_봉수}.pkl'))
            ...
            # 결과파일 저장
            Tool.df저장(df=df_수익내역, path=os.path.join(folder_타겟, f'{file_타겟}_{s_일자}_{s_봉수}'))
            ...
            n_총거래수 = df_누적리포트['거래수'].values[0] if n_일수 > 0 else 0
            n_일평균수익률 = df_누적리포트['수익률sum'].values[0] / n_일수 if n_일수 > 0 else 0
            n_기대수익 = df_누적리포트['기대수익'].values[0] if n_일수 > 0 else 0
            n_수익금액 = df_누적리포트['수익금액'].values[0] if n_일수 > 0 else 0
            n_금액수익률 = n_수익금액 / df_누적리포트['시작금액'].values[0] * 100 if n_일수 > 0 else 0
            self.make_로그(f'{s_일자} 완료\n'
                          f' - 총거래수 {n_총거래수:,.0f}건, 평균수익 {n_일평균수익률:,.1f}%, 기대수익 {n_기대수익:,.2f}\n'
                          f' - 수익금액 {n_수익금액:,.0f}원 ({n_금액수익률:,.1f}%)')

    def make_매매일보(self, s_봉수, b_카톡):
        ...
        for s_일자 in li_대상일자:
            ...
            df_일별 = df_일별.loc[:, ['일자', '거래수', '수익률sum', '이익거래수', '평균이익률', '손실거래수', '평균손실률',
                                  '승률', '손익비', '기대수익', '시작금액', '종료금액', '수익금액']]
            df_일별['수익률'] = (df_일별['종료금액'] / df_일별['시작금액'] - 1) * 100

            # 데이터 생성 - 누적
            df_거래별 = dic_수익내역['수익금액'].sort_values(['일자', '시간'])
            li_dic누적 = []
            for s_누적일자 in df_거래별['일자'].unique():
                df_누적거래 = df_거래별.loc[df_거래별['일자'] <= s_누적일자]
                df_누적거래_이익 = df_누적거래.loc[df_누적거래['수익률'] > 0]
                df_누적거래_손실 = df_누적거래.loc[df_누적거래['수익률'] < 0]
                n_총거래수 = len(df_누적거래)
                n_이익거래수 = len(df_누적거래_이익)
                n_손실거래수 = len(df_누적거래_손실)
                n_평균이익률 = df_누적거래_이익['수익률'].mean() if n_이익거래수 > 0 else 0
                n_평균손실률 = df_누적거래_손실['수익률'].mean() if len(df_누적거래_손실) > 0 else 0
                n_승률 = n_이익거래수 / n_총거래수 if n_총거래수 > 0 else 0
                n_손익비 = n_평균이익률 / abs(n_평균손실률) if n_평균손실률 != 0 else 10
                n_기대수익 = (n_승률 * n_손익비) - (1 - n_승률)
                n_시작금액 = df_누적거래['시작금액'].iloc[0]
                n_종료금액 = df_누적거래['종료금액'].iloc[-1]
                n_수익금액 = n_종료금액 - n_시작금액
                n_누적수익률 = (n_종료금액 / n_시작금액 - 1) * 100 if n_시작금액 != 0 else 0
                li_dic누적.append(dict(일자=s_누적일자, 누적거래수=n_총거래수, 누적수익률sum=df_누적거래['수익률'].sum(),
                                     누적이익거래수=n_이익거래수, 누적평균이익률=n_평균이익률, 누적손실거래수=n_손실거래수, 누적평균손실률=n_평균손실률,
                                     누적승률=n_승률, 누적손익비=n_손익비, 누적기대수익=n_기대수익,
                                     누적시작금액=n_시작금액, 누적종료금액=n_종료금액, 누적수익금액=n_수익금액,누적수익률=n_누적수익률))
            df_누적 = pd.DataFrame(li_dic누적).sort_values('일자').reset_index(drop=True)

            # 매매일보 생성
            df_누적_일자제외 = df_누적.loc[:, [컬럼 for 컬럼 in df_누적.columns if 컬럼 != '일자']]
            df_매매일보 = pd.concat([df_일별, df_누적_일자제외], axis=1, ignore_index=False)
            df_매매일보 = df_매매일보.loc[:, ['일자', '거래수', '누적거래수', '수익률sum', '누적수익률sum',
                                      '이익거래수', '누적이익거래수', '평균이익률', '누적평균이익률',
                                      '손실거래수', '누적손실거래수', '평균손실률', '누적평균손실률',
                                      '승률', '누적승률', '손익비', '누적손익비', '기대수익', '누적기대수익',
                                      '시작금액', '종료금액', '수익금액', '누적수익금액', '수익률', '누적수익률']]
            ...
            # 그래프 저장
            folder_그래프 = f'{folder_타겟}_그래프'
            file_그래프 = f'{file_타겟}_{s_일자}_{s_봉수}.svg'
            ...
            # 로그 기록
            n_일거래수 = df_매매일보['거래수'].values[-1]
            n_누적승률 = df_매매일보['누적승률'].values[-1]
            n_누적손익비 = df_매매일보['누적손익비'].values[-1]
            n_누적기대수익 = df_매매일보['누적기대수익'].values[-1]
            n_누적수익률 = df_매매일보['누적수익률'].values[-1]
            self.make_로그(f'{s_일자} 완료\n'
                          f' - 일거래 {n_일거래수}건, 누적승률 {n_누적승률 * 100:,.0f}%, 누적손익비 {n_누적손익비:,.1f}\n'
                          f' - 기대수익 {n_누적기대수익:,.2f}, 누적수익률 {n_누적수익률:,.0f}%')
    ...
    def _make_매매신호_종목(dic_매개변수):
        ...
        df_데이터['종가ma20'] = df_데이터['종가'].rolling(20).mean()
        df_데이터['종가ma60'] = df_데이터['종가'].rolling(60).mean()
        ...
        df_기준봉, df_기준봉전체 = pd.DataFrame(), pd.DataFrame()
        dic_매매신호 = dict()
        # for dt_시점 in df_데이터.index:
        ...
        os.makedirs(folder, exist_ok=True)
        df_매매신호.to_csv(os.path.join(folder, f'{file_타겟}_{s_일자}_{s_봉수}_{s_종목코드}_{s_종목명}.csv'),
    ...
    def _generate_누적리포트(self, dic_매개변수):
        ...
        # 매매내역 불러오기
        li_파일명 = sorted(파일 for 파일 in os.listdir(folder_타겟) if file_타겟 in 파일 and f'{s_봉수}' in 파일
    ...
    def _generate_수익금액(df_매매내역):
        ...
        df_매매내역 = df_매매내역.sort_values(['일자', '매수시간', '매도시간']).reset_index(drop=True)\
                    if not df_매매내역.empty else df_매매내역
        ...
        for idx in df_매매내역.index:
            ...
            n_매수금액 = n_매수가 * n_매매수량
            n_매도금액 = n_매도가 * n_매매수량 - int(n_매수금액 * 0.2 / 100)
            n_수익금액 = n_매도금액 - n_매수금액
            n_거래후자본금 = n_자본금 + n_수익금액

            # 수익금액 계산 - 자금관리 미적용
            n_올인매매수량 = int(n_올인자본금 / n_매수가)
            n_올인매수금액 = n_매수가 * n_올인매매수량
            n_올인매도금액 = n_매도가 * n_올인매매수량 - int(n_올인매수금액 * 0.2 / 100)
            n_올인수익금액 = n_올인매도금액 - n_올인매수금액
            n_올인거래후자본금 = n_올인자본금 + n_올인수익금액
            ...
            dic_수익금액_항목.update(한계리스크율=n_한계리스크율, 한계리스크=n_한계리스크, 매매수량=n_매매수량,
                               ...
                               올인매수금액=n_올인매수금액, 올인매도금액=n_올인매도금액,
                               올인시작금액=n_올인자본금, 올인종료금액=n_올인거래후자본금, 올인수익금액=n_올인수익금액)
    ...
    @staticmethod
    def _ax_기대수익(ax_승률손익비, ax_기대수익, df_매매일보, n_이동평균):
        ...
        ary_일별기대수익ma = df_매매일보['기대수익'].rolling(n_이동평균).mean().values
        ary_누적승률 = df_매매일보['누적승률'].values
        ary_누적손익비 = df_매매일보['누적손익비'].values
        ary_누적기대수익 = df_매매일보['누적기대수익'].values
        ...
        return (ax_승률손익비, ax_기대수익)

    @staticmethod
    def _ax_누적수익률(ax_누적수익률, df_매매일보):
        ...
        ary_누적수익률 = df_매매일보['누적수익률'].values
    ...
    def _ax_거래분봉(ax_거래분봉, df_분봉, dic_거래):
        ...
        # 차트 생성
        li_일시 = Chart.make_캔들차트(ax=ax_거래분봉, df_차트=df_분봉, s_봉구분=f'{n_분봉수}분봉', s_차트구분='캔들', b_legend=False)

        # 뷰 설정
        ax_거래분봉.set_title(f'[{n_분봉수}분봉] {s_종목명} | {s_매도사유}({n_수익률:.1f}%)'
...
def run():
    ...
    ret = [a.make_매수매도(s_봉수=봉수) for 봉수 in li_봉수] # 매수매도 신호 존재하는 종목의 데이터만 수집
    ret = [a.make_매매내역(s_봉수=봉수) for 봉수 in li_봉수] # 매매내역 정보만 한줄로 표기
    ret = [a.make_수익내역(s_봉수=봉수) for 봉수 in li_봉수] # 시간중복 제거 후 실매매 정보 표기
    ret = [a.make_매매일보(s_봉수=봉수, b_카톡=True) for 봉수 in li_봉수] # 시간중복 제거 후 실매매 정보 표기
