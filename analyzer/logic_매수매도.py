...
def judge_매수신호(df_기준봉):
    ...
    # b_배열검증 = n_종가 > n_종가ma20 > n_종가ma60
    # b_배열검증 = n_종가 > n_종가ma120
    # b_배열검증 = n_종가 > n_종가ma20 > n_종가ma60 > n_종가ma120
    b_배열검증 = n_종가 > n_종가ma20 > n_종가ma120
    ...
    # b_돌파검증 = (n_ATR비율차이 > 0.005) and (n_종가 < n_고가20 * 0.998) and (n_종가 < n_종가ma60 * 1.015)
    # b_돌파검증 = (n_ATR비율차이 > 0) and (n_종가 > n_고가20) and (n_종가 > n_종가ma60)
    # b_돌파검증 = n_종가 > n_고가20
    # b_돌파검증 = (n_종가 > n_고가20) and (n_바디 < 2)
    # b_돌파검증 = (n_종가 > n_고가5) and (n_바디 < 2)
    # b_돌파검증 = (n_종가 > n_고가5) and (n_바디 < 1)
    # b_돌파검증 = (n_종가 > n_고가5) and (n_바디 < 3)
    ...
    # b_물량검증 = (n_매수량 > 5 * n_매수량ma20) and (n_매수대금 > n_봉수 * 1 * 10**8)
    # b_물량검증 = n_ATR > 3 * n_ATR14
    # b_물량검증 = n_ATR > 2 * n_ATR14
    # b_물량검증 = n_거래량 > n_거래량ma20 * 1.6
    # b_물량검증 = n_거래량 > n_거래량ma20 * 2
    # b_물량검증 = n_매수대금 > n_봉수 * 1 * 10**8
    b_물량검증 = True
...
def judge_매도신호(df_기준봉, dic_args):
    ...
    n_고가보유 = df_1초봉.loc[df_1초봉['체결시간'] >= s_매수시간, '고가'].max()
    ...
    # 고가이탈 - 매수이후 고가 - 변동폭
    # n_고가보유 = df_기준봉전체.loc[df_기준봉전체['시간'] >= s_매수시간]['고가'].max()
    # n_추세변동 = 1 * n_ATR14
    # n_추세변동 = 3 * n_ATR14
    ...
    # b_추세이탈 = n_종가 < n_추세한계
    # b_추세이탈 = n_현재가 < n_추세한계
    # b_추세이탈 = (n_종가 < n_저가3) and (n_종가 - n_시가 < 0)
    # b_추세이탈 = n_종가 < n_저가3
    # b_추세이탈 = False
    n_고가변동 = 2 * n_ATR14
    # n_고가변동 = 3 * n_ATR14
    n_고가한계 = n_고가보유 - n_고가변동
    b_고가이탈 = n_현재가 < n_고가한계 and n_수익률 > 0
    li_매도신호.append(b_고가이탈)
    # li_매도신호.append(False)
    dic_매도신호.update(매도봇_n_고가보유=n_고가보유, 매도봇_n_고가변동=n_고가변동, 매도봇_n_고가한계=n_고가한계)

    # 손실한계 - 매수가 - 변동폭
    # n_손실기준 = n_매수가 - 2 * n_ATR14_매수
    n_손실기준 = n_매수가 - n_리스크
    # b_손실한계 = n_종가 < n_손실기준
    # b_손실한계 = n_현재가 < n_손실기준
    ...
    # b_타임아웃 = (n_경과시간 > 600 and n_거래량 > 0) or s_탐색시간 > '15:15:00'
    # b_타임아웃 = (n_경과시간 > 150 and n_거래량 > 0) or s_탐색시간 > '15:15:00'
    # b_타임아웃 = (n_경과시간 > 3 * 60 * 10 and n_거래량 > 0) or s_탐색시간 > '15:15:00'
    # b_시간경과 = n_경과시간 > 300 if '초봉' in s_봉수 else\
    #             n_경과시간 > 1800 if '분봉' in s_봉수 else False
