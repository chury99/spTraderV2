...
class CollectorBot:
    def __init__(self):
        ...
        # 추가 폴더 정의
        self.folder_임시저장 = os.path.join(self.folder_차트수집, '임시저장')
        self.folder_일봉 = os.path.join(self.folder_차트수집, '일봉')
        self.folder_분봉 = os.path.join(self.folder_차트수집, '분봉')
        os.makedirs(self.folder_임시저장, exist_ok=True)
        os.makedirs(self.folder_일봉, exist_ok=True)
        os.makedirs(self.folder_분봉, exist_ok=True)

        # 기준정보 정의
        self.s_오늘 = pd.Timestamp.now().strftime('%Y%m%d')
        # self.path_제외일자 = os.path.join(self.folder_임시저장, 'li_제외일자.pkl')
        # self.li_제외일자 = pd.read_pickle(self.path_제외일자) if os.path.exists(self.path_제외일자) else list()
    ...
    def find_대상일자(self):
        ...
        for s_봉구분 in dic_최종일자.keys():
            ...
            # 최종일자 확인 - 일봉
            if s_봉구분 == '일봉':
                s_테이블명_최종월 = max(li_테이블명)
                df_최종월 = Tool.sql불러오기(path=path_최종파일, s_테이블명=s_테이블명_최종월)
                dic_최종일자['일봉'] = df_최종월['일자'].max()

            # 최종일자 확인 - 분봉
            elif s_봉구분 == '분봉':
                s_테이블명_최종일자 = max(li_테이블명)
                dic_최종일자['분봉'] = re.findall(r'\d{8}', s_테이블명_최종일자)[0]

        # 전체일자 확인
        # li_전체일자 = [re.findall(r'\d{8}', 파일)[0] for 파일 in os.listdir(self.folder_전체종목) if '.pkl' in 파일]
        # li_전체일자 = [일자 for 일자 in li_전체일자 if 일자 not in self.li_제외일자]
        # li_전체일자 = sorted(li_전체일자)
        ...
        dic_대상일자 = dict()
        for s_봉구분 in dic_최종일자.keys():
            # li_일자_대상 = [일자 for 일자 in li_전체일자 if 일자 > dic_최종일자[s_봉구분]]
            # dic_대상일자[s_봉구분] = min(li_일자_대상) if len(li_일자_대상) > 0 else None
    ...
    def find_진행현황확인(self):
        """ 진행 중인 pkl 파일에서 진행현황 확인 """
        pass
    ...
    def get_차트데이터(self):
        ...
        for s_봉구분 in self.dic_li대상일자.keys():
            ...
            for s_대상일자 in self.dic_li대상일자[s_봉구분]:
                # 기준정보 정의
                path_차트정보 = os.path.join(self.folder_임시저장, f'dic_차트정보_{s_봉구분}_{s_대상일자}.pkl')
                ...
                for i, s_종목코드 in enumerate(li_잔여종목):
                    # # 데이터 미존재 처리 - 제외일자 업데이트 후 종료
                    # if i == 30 and len(dic_차트정보['li_제외종목']) >= i:
                    #     self.li_제외일자 = self.li_제외일자 + [s_대상일자]
                    #     pd.to_pickle(self.li_제외일자, self.path_제외일자)
                    #     break
                ...
                # if len(li_df차트) > 0:
                #     df_차트 = pd.concat(li_df차트, axis=0)
                #     li_컬럼명 = ['일자', '종목코드', '종목명'] + [컬럼 for 컬럼 in df_차트.columns if 컬럼 not in ['일자', '종목코드']]
                #     dic_차트정보['df_차트'] = df_차트[li_컬럼명]
                #
                # # 데이터 저장 - 신뢰성 확인 병행
                # n_수집 = len(dic_차트정보['df_차트'])
                # n_제외 = len(dic_차트정보['li_제외종목'])
                # n_전체종목 = len(dic_차트정보['li_전체종목'])
                # if n_수집 + n_제외 == n_전체종목:
                #     pd.to_pickle(dic_차트정보, path_차트정보)
                # else:
                #     with open(os.path.join(self.folder_임시저장, f'error_신뢰성_{s_봉구분}_{s_대상일자}.txt')) as f:
                #         f.write(f'--- {s_봉구분}-{s_대상일자} ---\n'
                #                 f'  수집-{n_수집:,.0f}항목\n'
                #                 f'  제외-{n_제외:,.0f}항목\n'
                #                 f'  수집+제외-{n_수집 + n_제외}\n'
                #                 f'  전체종목-{n_전체종목}\n')
